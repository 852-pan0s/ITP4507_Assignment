...
public class CIMSTool {
	...
	public void run() {
		...
		Stack<Memoento> undoSaves = new Stack<Memoento>();
		Stack<Memoento> redoSaves = new Stack<Memoento>();
		...
		while (true) {
			...
			input = sc.nextLine();
			if (input.equals("a")) {
				c = new AddCommand(undoList, undoSaves, undoProducts, products, sc);
			} else if (input.equals("v")) {
				c = new ViewCommand(products, sc);
			} else if (input.equals("c")) {
				c = new CollectCommand(undoList, undoSaves, undoProducts, products, sc);
			} else if (input.equals("s")) {
				c = new ShipCommand(undoList, undoSaves, undoProducts, products, sc);
			} else if (input.equals("u")) {
				c = new UndoCommand(undoList, redoList, undoSaves, redoSaves, undoProducts, redoProducts, products);
			} else if (input.equals("r")) {
				c = new RedoCommand(undoList, redoList, undoSaves, redoSaves, undoProducts, redoProducts, products);
